...
from data import high_low_data, Dataset
...
def experiment(lr, weight_decay, train_list, eval_list, args, device):
    ...
    eeg_train_dataset = high_low_data(train_list, eval_list, person=args.person, feature=args.feature, type='train')
    ...
    for lr in [1e-3, 1e-4, 1e-5]:
        for weight_decay in [1e-2, 1e-4, 1e-6]:
            ...
            if acc > max_accuracy:
                max_accuracy = acc
                max_model = model
    directory = "high_low_dc_nocrossatten/{}/{}/{}".format(args.person, args.feature, args.eval_folder)
