...
class DCmodel2(nn.Module):
    def __init__(
        ...
        self.vq = VQEmbedding(n_embeddings=400, embedding_dim=embed_dim)
        self.discrete_attention = CrossAttention(embed_dim=embed_dim, num_heads=4)
    ...
    def no_weight_decay(self):
        return {"eeg_pos_embed",  "eeg_cls_token",  "eeg_type_embed"}

    def forward_features(self, eeg): 
        eeg = self.eeg_transform(eeg)
        discrete_x = self.vq(eeg)
        ...
        x = self.discrete_attention(discrete_x, x, x)
        x = torch.mean(x, dim=1)
        discrete_x = torch.mean(discrete_x, dim=1)

        fusion = torch.cat([x, discrete_x], dim=1)
        out = self.head(fusion)
        return out
          

    def forward(self, eeg=None):
